...
## [1.8.0] - 2026-04-10
- Fixed NullReferenceException when clicking "Run Selected" in the Test Runner with no tests visible in search results (UUM-143367).
- Fixed an issue where parameterized tests with long names that get truncated by NUnit could produce duplicate unique IDs, causing "An item with the same key has already been added" errors in the Test Runner UI (UUM-107986).
- Fixed an issue where `[UnityOneTimeSetUp]` was called a second time just before `[UnityOneTimeTearDown]` when the test fixture contained parameterized tests (UUM-130845).
- Fixed an issue where domain reloads triggered inside UnityOneTimeSetUp or UnityOneTimeTearDown were not executed, causing assemblies to not be reloaded correctly (UUM-114078).
- Added `ITestCommandWrapper` interface (with `Order` property) and `TestCommandWrapperRegistry` for external packages to wrap test commands with custom behavior.
- Made `IEnumerableTestMethodCommand` interface public to support coroutine-style test command wrappers.

## [1.7.1] - 2026-03-17
- Added support for internal platform-specific performance test config

## [1.7.0] - 2025-12-11
- Added the `UnityCoreClrExplicitDisabled` attribute to disable tests for the migration to CoreClr. It's for internal use only
...
- Fixed a stack overflow exception that might happen with some assembly setups with precompiled assemblies (UUM-122047).

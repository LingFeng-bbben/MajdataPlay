...
namespace UnityEngine.TestRunner.NUnitExtensions.Runner
{
    internal class CompositeWorkItem : UnityWorkItem
    {
...
#if !UNITY_WEBGL
        private List<OneTimeMethodLevel> _oneTimeLevels;
        private bool _hasAsyncOneTimeMethods;
#endif
                        ...
                        case RunState.Explicit:
                            ...
                            if (Children.Count > 0)
                            {
                                if (!m_DontRunRestoringResult)
                                {
                                    ...
                                    foreach (var child in ExecuteWithEditModeTestYieldHandling(
                                        _unityOneTimeSetupTearDownCommand.ExecuteOneTimeSetUpEnumerable(Context)))
                                        yield return child;

#if !UNITY_WEBGL
                                    if (_hasAsyncOneTimeMethods)
                                    {
                                        PerformOneTimeSetUp();
                                ...
                                if (Context.ExecutionStatus != TestExecutionStatus.AbortRequested && !m_DontRunRestoringResult)
                                {
                                    ...
                                    {
                                        foreach (var child in ExecuteWithEditModeTestYieldHandling(
                                            PerformOneTimeTearDownEnumerable()))
                                            yield return child;
                                    ...
                                    foreach (var child in ExecuteWithEditModeTestYieldHandling(
                                        _unityOneTimeSetupTearDownCommand.ExecuteOneTimeTeardownEnumerable(Context)))
                                        yield return child;
                                }
                            }
                            break;
...
#if !UNITY_WEBGL
            if (_suite.TypeInfo != null && HasAnyAsyncOneTimeMethods(_suite.TypeInfo.Type))
            {
                _hasAsyncOneTimeMethods = true;
                _oneTimeLevels = BuildOneTimeMethodLevels(_suite.TypeInfo.Type);
                setUpTearDownItems = new List<SetUpTearDownItem>();
            }
            else
#endif
            if (_suite.TypeInfo != null)
            {
                setUpTearDownItems = CommandBuilder.BuildSetUpTearDownList(_suite.TypeInfo.Type, typeof(OneTimeSetUpAttribute), typeof(OneTimeTearDownAttribute));
            }
            else
            {
                setUpTearDownItems = new List<SetUpTearDownItem>();
...
#if !UNITY_WEBGL
        private static bool HasAnyAsyncOneTimeMethods(Type fixtureType)
        {
            for (var type = fixtureType; type != typeof(object) && type != null; type = type.BaseType)
            {
                if (Reflect.GetMethodsWithAttribute(type, typeof(OneTimeSetUpAttribute), false)
                    .Any(m => m.ReturnType == typeof(Task)))
                    return true;
                if (Reflect.GetMethodsWithAttribute(type, typeof(OneTimeTearDownAttribute), false)
                    .Any(m => m.ReturnType == typeof(Task)))
                    return true;
            }
            return false;
        ...
        private static List<OneTimeMethodLevel> BuildOneTimeMethodLevels(Type fixtureType)
        {
            var levels = new List<OneTimeMethodLevel>();
            for (var type = fixtureType; type != typeof(object) && type != null; type = type.BaseType)
            {
                var setUpMethods = Reflect.GetMethodsWithAttribute(type, typeof(OneTimeSetUpAttribute), false).ToList();
                var tearDownMethods = Reflect.GetMethodsWithAttribute(type, typeof(OneTimeTearDownAttribute), false).ToList();
                if (setUpMethods.Count > 0 || tearDownMethods.Count > 0)
                {
                    levels.Add(new OneTimeMethodLevel
                    {
        ...
        private void EnsureFixtureCreated()
        {
            if (_suite.TypeInfo == null || _suite.TypeInfo.IsStaticClass)
                return;

            if (_suite.Fixture == null)
            {
                Context.TestObject = _suite.TypeInfo.Construct(_suite.Arguments);
                _suite.Fixture = Context.TestObject;
            }
            else
            {
                Context.TestObject = _suite.Fixture;
            }
        }

        private IEnumerator InvokeOneTimeMethodEnumerator(MethodInfo method)
        {
            var target = method.IsStatic ? null : Context.TestObject;
            if (method.ReturnType == typeof(Task))
            {
                var task = (Task)Reflect.InvokeMethod(method, target);
            ...
            {
                Reflect.InvokeMethod(method, target);
            }
        }

        private IEnumerable PerformOneTimeSetUpEnumerable()
        {
            EnsureFixtureCreated();

            foreach (var level in _oneTimeLevels)
            {
                bool levelFailed = false;

                foreach (var method in level.SetUpMethods)
                {
                    var logScope = new LogScope();
                    var enumerator = InvokeOneTimeMethodEnumerator(method);
                    ...
                    while (true)
                    {
                        ...
                        {
                            logScope.Dispose();
                            Result.RecordException(caught, FailureSite.SetUp);
                            levelFailed = true;
                            break;
                        }
                ...
                level.SetUpCompleted = true;
            }
        }

        private IEnumerable PerformOneTimeTearDownEnumerable()
        {
            for (int i = _oneTimeLevels.Count - 1; i >= 0; i--)
            {
                var level = _oneTimeLevels[i];
                if (!level.SetUpCompleted)
                    continue;

                for (int j = level.TearDownMethods.Count - 1; j >= 0; j--)
                {
                    var method = level.TearDownMethods[j];
                    var logScope = new LogScope();
                    var enumerator = InvokeOneTimeMethodEnumerator(method);
                    ...
                    while (true)
                    {
                        ...
                        {
                            logScope.Dispose();
                            Result.RecordException(caught, FailureSite.TearDown);
                            methodFailed = true;
                            break;
                        }
        ...
        private class OneTimeMethodLevel
        {
        ...
        private IEnumerable ExecuteWithEditModeTestYieldHandling(IEnumerable enumerable)
        {
            foreach (var item in enumerable)
            {
                if (item is IEditModeTestYieldInstruction editModeInstruction)
                {
                    yield return editModeInstruction;
                    var enumerator = editModeInstruction.Perform();
                    while (true)
                    {
                        bool moveNext;
                        try

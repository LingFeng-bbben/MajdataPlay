    ...
    internal class TestSettings : ITestSettings
    {
        private readonly TestSetting[] m_Settings =
        {
            ...
            new TestSetting<ScriptingImplementation?>(
                ...
                implementation =>
                {
                    if (implementation.HasValue)
                        PlayerSettings.SetScriptingBackend(NamedBuildTarget.FromBuildTargetGroup(EditorUserBuildSettings.activeBuildTargetGroup), implementation.Value);
                }),
#else
            new TestSetting<ScriptingImplementation?>(
                settings => settings.scriptingBackend,
            ...
            new TestSetting<ApiCompatibilityLevel?>(
                ...
                implementation =>
                {
                    if (implementation.HasValue && Enum.IsDefined(typeof(ApiCompatibilityLevel), implementation.Value))
                    {

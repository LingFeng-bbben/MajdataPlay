...
namespace LibUsbDotNet.WinUsb
{
	...
	{
		public override DriverModeType DriverMode => DriverModeType.LibusbK;

		internal LibusbKDevice(UsbApiBase usbApi, SafeFileHandle usbHandle, SafeHandle handle, string devicePath)
		...
		{
			bool num = LibusbKAPI.UsbK_SetCurrentAlternateSetting(mUsbHandle, (byte)alternateID);
			if (!num)
			{
		...
		public override bool GetAltInterface(out int alternateID)
		{
			alternateID = -1;
			byte SettingNumber;
			bool num = LibusbKAPI.UsbK_GetCurrentAlternateSetting(mUsbHandle, out SettingNumber);
			if (!num)
			{
		...
		public override bool Open()
		{
			...
			bool flag = LibusbKAPI.OpenDevice(out sfhDevice, mDevicePath);
			if (flag)
			{
				SafeLibusbKInterfaceHandle InterfaceHandle = new SafeLibusbKInterfaceHandle();
				if (flag = LibusbKAPI.UsbK_Initialize(sfhDevice, ref InterfaceHandle))
				{
		...
		public static bool Open(string devicePath, out LibusbKDevice usbDevice)
		{
			usbDevice = null;
			SafeFileHandle sfhDevice;
			bool flag = LibusbKAPI.OpenDevice(out sfhDevice, devicePath);
			if (flag)
			{
				SafeLibusbKInterfaceHandle InterfaceHandle = new SafeLibusbKInterfaceHandle();
				flag = LibusbKAPI.UsbK_Initialize(sfhDevice, ref InterfaceHandle);
				if (flag)
				{
					usbDevice = new LibusbKDevice(UsbDevice.LibusbKApi, sfhDevice, InterfaceHandle, devicePath);
				}
				else
				{
					UsbError.Error(ErrorCode.Win32Error, Marshal.GetLastWin32Error(), "Open:Initialize", typeof(UsbDevice));
		...
		public override bool GetAssociatedInterface(byte associatedInterfaceIndex, out WindowsDevice usbDevice)
		{
			...
			bool num = LibusbKAPI.UsbK_GetAssociatedInterface(mUsbHandle, associatedInterfaceIndex, ref AssociatedInterfaceHandle);
			if (num)
			{
				usbDevice = new LibusbKDevice(handle: new SafeLibusbKInterfaceHandle(AssociatedInterfaceHandle), usbApi: mUsbApi, usbHandle: null, devicePath: mDevicePath);
			}
			if (!num)
			{
		...
		public override bool QueryDeviceSpeed(out DeviceSpeedTypes deviceSpeed)
		{
			...
			int BufferLength = 1;
			bool num = LibusbKAPI.UsbK_QueryDeviceInformation(mUsbHandle, DeviceInformationTypes.DeviceSpeed, ref BufferLength, array);
			if (num)
			{
		...
		{
			bool num = LibusbKAPI.UsbK_QueryInterfaceSettings(base.Handle, alternateInterfaceNumber, usbAltInterfaceDescriptor);
			if (!num)
			{
		...
		{
			bool num = LibusbKAPI.UsbK_GetPowerPolicy(mUsbHandle, policyType, ref valueLength, pBuffer);
			if (!num)
			{
		...
		{
			return LibusbKRegistry.GetDevicePathList(interfaceGuid, out devicePathList);
		}

		internal override bool SetPowerPolicy(PowerPolicyType policyType, int valueLength, IntPtr pBuffer)
		{
			bool num = LibusbKAPI.UsbK_SetPowerPolicy(mUsbHandle, policyType, valueLength, pBuffer);
			if (!num)
			{

...
public class SlideDrop : NoteLongDrop, IFlasher
{
    ...
    private void FixedUpdate()
    {
        ...
        var startTiming = gamePlayManager.AudioTime - timeStart;
        ...
        if (ConnectInfo.IsConnSlide)
        {
            ...
            else if (ConnectInfo.IsGroupPartEnd && gamePlayManager.AudioTime - forceJudgeTiming >= 0)
                TooLateJudge();
            else if(isFinished)
                HideBar(areaStep.LastOrDefault());
        }
        ...
        else if (gamePlayManager.AudioTime - forceJudgeTiming >= 0)
            TooLateJudge();
    }
    ...
    private void Update()
    {
        ...
        var startiming = gamePlayManager.AudioTime - timeStart;
        if (startiming <= 0f)
        {
    ...
    void Judge()
    {
        ...
        if (!isJudged)
        {
            arriveTime = gamePlayManager.AudioTime;
            var triggerTime = gamePlayManager.AudioTime;           
        ...
        else if (arriveTime < starTiming && gamePlayManager.AudioTime >= starTiming + stayTime * 0.8)
            DestroySelf();
        else if (arriveTime >= starTiming && gamePlayManager.AudioTime >= arriveTime + stayTime * 0.8)
            DestroySelf();
    }
    /// <summary>
